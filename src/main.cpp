...
{
    // ------------------------------------------------------------------
    // Generate and bind the framebuffer for Step 1 scene offscreen render
    glGenFramebuffers(1, &colourFBO);
    glBindFramebuffer(GL_FRAMEBUFFER, colourFBO);

    // Create the texture object for storing the scene render
    ...
    // Create the depth buffer to the framebuffer
    GLuint depthBuf;
    glGenRenderbuffers(1, &depthBuf);
    glBindRenderbuffer(GL_RENDERBUFFER, depthBuf);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, width, height);

    // Attach the depth buffer to colourFBO
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depthBuf);

    // ------------------------------------------------------------------
    // Create an FBO for Step 2,3,4 the bright-pass filter and blur
    ...
    // Downsampling is equivalent to box blur
